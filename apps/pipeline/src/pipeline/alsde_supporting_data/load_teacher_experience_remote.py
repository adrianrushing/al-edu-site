...
FILE_PREFIX = "alsde_educator_experience_"
...
def _ensure_target_table(conn: psycopg.Connection) -> None:
    ...
    CREATE TABLE IF NOT EXISTS staging.stg_teacher_experience (
        year TEXT,
        system TEXT,
        school TEXT,
        gender TEXT,
...
def _dedup_existing_rows(conn: psycopg.Connection) -> None:
    ...
    dedup_sql = f"""
    DELETE FROM staging.stg_teacher_experience t
    USING (
        SELECT ctid,
               ROW_NUMBER() OVER (
...
def _ensure_unique_index(conn: psycopg.Connection) -> None:
    index_sql = """
    CREATE UNIQUE INDEX IF NOT EXISTS stg_teacher_experience_unique_idx
    ON staging.stg_teacher_experience (
        year, system, school, gender, race, ethnicity, sub_population,
        total_count, experienced_count, experienced_rate,
        inexperienced_count, inexperienced_rate
    );
    """
    with conn.cursor() as cur:
...
def _copy_to_temp(conn: psycopg.Connection, rows: list[tuple[str | None, ...]]) -> None:
    copy_sql = (
        "COPY staging.tmp_stg_teacher_experience_load ("
        + ", ".join(TARGET_COLUMNS)
        + ") FROM STDIN"
    )
    with conn.cursor() as cur:
...
def _flush_batch(conn: psycopg.Connection, batch: list[tuple[str | None, ...]]) -> int:
    ...
    insert_sql = f"""
        INSERT INTO staging.stg_teacher_experience ({", ".join(TARGET_COLUMNS)})
        SELECT DISTINCT {", ".join(TARGET_COLUMNS)}
        FROM staging.tmp_stg_teacher_experience_load
        ON CONFLICT ({", ".join(UNIQUE_COLUMNS)}) DO NOTHING;
    """
    with conn.cursor() as cur:
        cur.execute(insert_sql)
        inserted = cur.rowcount or 0
        cur.execute("TRUNCATE staging.tmp_stg_teacher_experience_load;")
    conn.commit()
    return inserted
...
def run_load(source_dir: Path, database_url: str, batch_size: int) -> None:
    ...
    if not files:
        raise RuntimeError(f"No yearly teacher experience files found in {source_dir}")
    ...
    with psycopg.connect(
        ...
        with conn.cursor() as cur:
            cur.execute("DROP TABLE IF EXISTS staging.tmp_stg_teacher_experience_load;")
            cur.execute(
                """
                CREATE TABLE staging.tmp_stg_teacher_experience_load
                (LIKE staging.stg_teacher_experience INCLUDING DEFAULTS);
                """
            )
        conn.commit()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Load combined ALSDE teacher experience yearly CSV files into "
            "staging.stg_teacher_experience with deduplicated inserts."
        )
    )
    parser.add_argument(
        "--source-dir",
...
def main() -> None:
    ...
    logger.info("teacher_experience_remote_load_complete=true")
