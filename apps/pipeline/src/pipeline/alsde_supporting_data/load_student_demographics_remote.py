...
def _ensure_target_table(conn: psycopg.Connection) -> None:
    ...
    CREATE TABLE IF NOT EXISTS staging.stg_student_demographics (
        year TEXT,
        system TEXT,
        school TEXT,
        grade TEXT,
...
def _dedup_existing_rows(conn: psycopg.Connection) -> None:
    ...
    dedup_sql = f"""
    DELETE FROM staging.stg_student_demographics t
    USING (
        SELECT ctid,
               ROW_NUMBER() OVER (
...
def _ensure_unique_index(conn: psycopg.Connection) -> None:
    index_sql = """
    CREATE UNIQUE INDEX IF NOT EXISTS stg_student_demographics_unique_idx
    ON staging.stg_student_demographics (
        year,
        system,
        school,
        grade,
...
def _iter_yearly_files(source_dir: Path) -> list[Path]:
    files = []
    for file_path in sorted(source_dir.glob("alsde_student_demographics_*.csv")):
        suffix = file_path.stem.removeprefix("alsde_student_demographics_")
...
def _copy_to_temp(conn: psycopg.Connection, rows: list[tuple[str | None, ...]]) -> None:
    copy_sql = (
        "COPY staging.tmp_stg_student_demographics_load ("
        + ", ".join(TARGET_COLUMNS)
        + ") FROM STDIN"
    )
    with conn.cursor() as cur:
...
def _flush_batch(conn: psycopg.Connection, batch: list[tuple[str | None, ...]]) -> int:
    ...
    insert_sql = f"""
        INSERT INTO staging.stg_student_demographics ({", ".join(TARGET_COLUMNS)})
        SELECT DISTINCT {", ".join(TARGET_COLUMNS)}
        FROM staging.tmp_stg_student_demographics_load
        ON CONFLICT ({", ".join(UNIQUE_COLUMNS)}) DO NOTHING;
    """
    with conn.cursor() as cur:
        cur.execute(insert_sql)
        inserted = cur.rowcount or 0
        cur.execute("TRUNCATE staging.tmp_stg_student_demographics_load;")
    conn.commit()
    return inserted
...
def run_load(source_dir: Path, database_url: str, batch_size: int) -> None:
    ...
    if not files:
        raise RuntimeError(f"No yearly student demographics files found in {source_dir}")
    ...
    with psycopg.connect(
        ...
        with conn.cursor() as cur:
            cur.execute("DROP TABLE IF EXISTS staging.tmp_stg_student_demographics_load;")
            cur.execute(
                """
                CREATE TABLE staging.tmp_stg_student_demographics_load
                (LIKE staging.stg_student_demographics INCLUDING DEFAULTS);
                """
            )
        conn.commit()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Load combined ALSDE student demographics yearly CSV files into "
            "staging.stg_student_demographics with deduplicated inserts."
        )
    )
    parser.add_argument(
        "--source-dir",
...
def main() -> None:
    ...
    yearly_files = _iter_yearly_files(source_dir)
    logger.info("source_dir=%s yearly_files=%s", source_dir, len(yearly_files))
    run_load(source_dir=source_dir, database_url=remote_url, batch_size=args.batch_size)

    logger.info("student_demographics_remote_load_complete=true")
